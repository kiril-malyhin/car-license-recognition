...
def process_image(name, debug, **options):
    ...
    for contour in contours:
        ...
        if validate_contour(contour, gray, aspect_ratio_range, area_range):
            ...
            if edge_density > 0.5:
                print(box)
                image = Image.open('tests/1.jpg')

                img = cv2.adaptiveThreshold(ed_img,255,cv2.ADAPTIVE_THRESH_MEAN_C,\
                            cv2.THRESH_BINARY,11,2)
                with PyTessBaseAPI() as api:
                    api.SetImage(img)
                    api.SetRectangle(box[1][0], box[1][1], box[3][0] - box[1][0], box[3][1] - box[1][1])
